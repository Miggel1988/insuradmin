...
public class MongoService {
	...
	public void addPolice(Police police) {
		getDatabase(database).getCollection(COLLECTION_POLICE, Police.class).insertOne(police);
	}
	
	
	public List<Police> loadPolicies(String customerID) {
		List<Police> lstPolice = new ArrayList<>();
		MongoCursor<Police> cursor = getDatabase(database).getCollection(COLLECTION_POLICE, Police.class).find(Filters.eq("customerID", customerID)).iterator();

		try {
			while (cursor.hasNext()) {
				lstPolice.add(cursor.next());
			}
		} finally {
			cursor.close();
		}
